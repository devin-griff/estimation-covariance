...
## Concurrency and solver licensing standards

Every app in the family follows these two rules. They exist because of how
the stack layers: each Fly machine runs ONE `streamlit run` process, and
Streamlit executes every connected user's rerun as a thread of that same
process. Threads share all module-level state.

### One solve at a time per machine (the solve lock)

Solver-log capture is process-global: Pyomo's `capture_output(capture_fd=True)`
redirects the process's stdout file descriptor, `contextlib.redirect_stdout`
swaps `sys.stdout`, and appsi solvers use the same tee machinery internally.
Two overlapping solves from two session threads corrupt each other's
redirect stack and fail BOTH solves (verified: concurrent appsi solves
raise `RuntimeError: Captured output does not match sys.stdout`). So every
solve serializes behind one process-wide lock:
...
The lock MUST come from `st.cache_resource`. Streamlit re-executes the
whole script for every rerun of every session, each time into a fresh
module namespace, so a bare module-level `threading.Lock()` is a new
object per rerun and serializes nothing. Only `st.session_state`,
`st.cache_data`/`st.cache_resource`, and imported modules survive
reruns; anything that must be shared process-wide goes through
`st.cache_resource`.

Demo-scale solves are seconds at most, so users never notice the queueing.
Apps that solve in a loop (gas-storage's Monte Carlo) take the lock per
inner solve, not per run, so two users' runs interleave fairly. Scale-out
is horizontal: more machines, plus an `[http_service.concurrency]` block in
`fly.toml` so Fly's proxy actually spreads users (without one, every user
lands on the first machine). Never concurrent solves within a process.

### Licensed solvers hold their session while awake

Gurobi WLS checkout is a network round trip (about 0.5-1 s). Apps pay it
once: the first solve after a machine wakes creates the environment, and
the session is held until Fly auto-stops the machine (about five minutes
after the last visitor). Do not call `release_license()` per solve. The
license allows 50 concurrent sessions; a handful of auto-stop machines
never gets close. Guard the wake-up checkout with one quiet retry, then
surface a `license_busy` status: see `_LicenseBusyError` and the retry
loop in plant-layout or facility-location.
...
The Gurobi pattern is established across the family: set the three WLS
values as Fly secrets (`GRB_WLSACCESSID`, `GRB_WLSSECRET`, `GRB_LICENSEID`),
add the license shim that writes `~/gurobi.lic` at app startup (copy
`_materialize_gurobi_license` from any Gurobi app), and follow the
hold-while-awake session policy above. Locally, `GRB_LICENSE_FILE` points
at the license file on disk and the shim no-ops.
