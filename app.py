...
# The 32px size lives on the img tag itself (attributes + inline style),
# not only in the CSS block above: the 512px favicon otherwise paints
# full-size for a frame whenever the image decodes before Streamlit
# mounts the custom stylesheet (an intermittent cold-load flash).
_HOME_LOGO_HTML = (
    '<a class="home-logo-corner" href="https://griffith-pse.com" target="_self">'
    f'<img src="{_FAVICON_DATA_URL}" '
        'alt="Griffith PSE: home" width="32" height="32" '
...
#
# If the app runs a solver, serialize solves behind one process-wide lock
# (one solve at a time per machine). Streamlit runs every user session as
# a thread of this one process, and solver-log capture redirects
# process-global stdout, so overlapping solves corrupt each other. The
# lock must come from st.cache_resource (a bare module-level Lock is a
# new object every rerun and serializes nothing):
#   @st.cache_resource(show_spinner=False)
#   def _solve_lock():
#       return threading.Lock()
#   with _solve_lock(), capture_output(capture_fd=True) as buf:
#       results = pyo.SolverFactory("appsi_highs").solve(m, tee=True)
# See SETUP.md "Concurrency and solver licensing standards".
#
# Example:
#   @st.cache_data
#   def fit_model(data):
